			...
			<div className=' flex justify-end pt-5 px-5 relative'>
				...
				{isMenu && (
					<div className=' absolute bg-gray-300 top-16 p-2 rounded space-y-2'>
						<div
							className=' p-1 select-none hover:bg-gray-400 rounded cursor-pointer'
							onClick={() => sortHandler('priceHighToLow')}
						>
							Price - High to Low
						</div>
						<div
							className=' p-1 select-none hover:bg-gray-400 rounded cursor-pointer'
							onClick={() => sortHandler('priceLowToHigh')}
						>
							Price - Low to High
						</div>
									...
									<div>
										{wishlistStatus === 'error' && (
											<p className='text-red-500'>
												{wishListError?.response?.data?.message || 'Something went wrong'}
											</p>
										)}
									</div>
								</div>
